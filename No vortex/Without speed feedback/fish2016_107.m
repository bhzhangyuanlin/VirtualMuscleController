...
theta_10f    = 0.1179;       %第一关节相对于鱼体头部的转动角度幅值
theta_21f    = 0.3344;       %第二关节相对于第一关节转动的角度幅值
...
for t = dt : dt : time                                                     %从0.01秒到10秒
   ...
   % f=f+0.0005*(f_g-f);                                                     %闭环控制f，使得f逼近f_g,从而实现U逼近v_g;
   f=f_g;
   Kp_1=Kp_1+1*(Kp_g-Kp_1);                                                %闭环控制Kp_1，使得Kp_1逼近Kp_g,从而实现U逼近v_g;
    Kp_2=50;
    v_g_t(counter)=v_g;
    f_g_t(counter)=f_g;                            
    x_axis(counter)=t;   
